...
class SimpleGraphClient {
  ...
  async sendMail(toAddress, subject, content) {
    if (!toAddress || !toAddress.trim()) {
      throw new Error(
        "SimpleGraphClient.sendMail(): Invalid `toAddress` parameter received."
      );
    }
    if (!subject || !subject.trim()) {
      throw new Error(
        "SimpleGraphClient.sendMail(): Invalid `subject`  parameter received."
      );
    }
    if (!content || !content.trim()) {
      throw new Error(
        "SimpleGraphClient.sendMail(): Invalid `content` parameter received."
      );
    }
